...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/16 16:29:22 by abastian          #+#    #+#             */
/*   Updated: 2024/10/17 16:33:59 by abastian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*image = mlx_xpm_file_to_image(game->mlx_ptr, path, &(game->map.img_size.x), &(game->map.img_size.y));
	if(!*image)
...
void	start_mlx(t_game *game)
{
	...
	game->map.total_size.y = (game->map.map_size.y + 1) * game->map.img_size.y; // + 1 car la map compte a partir de 0;
	game->mlx_ptr = mlx_init();
	if (!game->mlx_ptr)
		error_game("Could not initiate the first mlx thing", game);
	game->win_ptr = mlx_new_window(game->mlx_ptr, game->map.total_size.x, game->map.total_size.y, "So_Long");
	if (!game->win_ptr)
		error_game("T en galere mon reuf", game);
	set_textures(game);
